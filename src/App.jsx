...
function App() {
  ...
  const loadImages = async (pageNumber = page) => {
    try {
      setLoadingImages(true);
      setError("");

      const res = await fetchImages(pageNumber);

      //  SAFETY CHECK
      const imagesData = Array.isArray(res.data) ? res.data : [];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex justify-center px-4 py-6">
      <div className="w-full max-w-6xl bg-slate-950/80 border border-slate-800 rounded-3xl shadow-2xl p-6 sm:p-8">
        ...
        {/* Form */}
        <form onSubmit={handleImport} className="space-y-3 mb-6 sm:mb-8">
